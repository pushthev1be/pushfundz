# 🚀 Railway Deployment Guide for PushFundz (Monorepo)
...
This is a monorepo with multiple services. You MUST create separate Railway services for each component and specify root directories.

## Step 1: Connect GitHub to Railway

1. Go to [railway.app](https://railway.app) and sign in with GitHub
2. Click "New Project" → "Empty Project" (NOT "Deploy from GitHub repo")
3. Give your project a name like "PushFundz"

## Step 2: Deploy Backend Service

1. **Create Backend Service:**
   - Click "Add Service" → "GitHub Repo"
   - Select your `pushfundz` repository
   - **CRITICAL**: Set "Root Directory" to: `crypto-lending-backend`
   - Railway will now auto-detect Python/Poetry in the backend folder
...
1. **Create Frontend Service:**
   - Click "Add Service" → "GitHub Repo"
   - Select your `pushfundz` repository again
   - **CRITICAL**: Set "Root Directory" to: `crypto-lending-frontend`
   - Railway will now auto-detect Node.js/React in the frontend folder
...
1. **Nixpacks Build Failed (Monorepo Issue):**
   - **Solution**: Make sure you set the "Root Directory" for each service
   - Backend service: Root Directory = `crypto-lending-backend`
   - Frontend service: Root Directory = `crypto-lending-frontend`
   - If you deployed without root directories, delete and recreate the services
...
- [ ] Created empty Railway project (not "Deploy from GitHub")
- [ ] Backend service has Root Directory: `crypto-lending-backend`
- [ ] Frontend service has Root Directory: `crypto-lending-frontend`
- [ ] PostgreSQL database service added
- [ ] Environment variables set for each service
- [ ] Database migrations run via console
