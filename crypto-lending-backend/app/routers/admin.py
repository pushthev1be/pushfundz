...
from ..database import get_db, User as DBUser, Loan as DBLoan, Membership as DBMembership, PointsLedger as DBPointsLedger
from ..schemas import LoanStatus, UserResponse, LoanResponse, AdminUserResponse, StatsResponse, AdminLoginRequest
from ..auth import get_admin_user, authenticate_user, create_access_token
...
async def get_negative_balance_users(
    admin: DBUser = Depends(get_admin_user),
    db: Session = Depends(get_db)
) -> Dict[str, Any]:
...
async def get_admin_stats(
    ...
    try:
        ...
        total_rp_earned = db.query(func.sum(DBPointsLedger.points_delta)).filter(
            DBPointsLedger.points_delta > 0
        ).scalar() or 0
        
        total_rp_spent = abs(db.query(func.sum(DBPointsLedger.points_delta)).filter(
            DBPointsLedger.points_delta < 0
        ).scalar() or 0)
        ...
        return {
            ...
            "loans": {
                ...
                "total_disbursed_usd": float(total_disbursed)
            },
            "financial": {
                "total_user_balance": float(total_balance),
                "platform_exposure": float(total_disbursed - total_balance)
            },
            "rp_economy": {
                "total_earned": int(total_rp_earned),
                "total_spent": int(total_rp_spent),
                "net_circulation": int(total_rp_earned - total_rp_spent)
            },
            "memberships": {
