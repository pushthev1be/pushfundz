...
class DailyLoginStreak(Base):
    __tablename__ = "daily_login_streak"
...
class LoanRPBenefit(Base):
    __tablename__ = "loan_rp_benefits"
    
    id = Column(GUID(), primary_key=True, default=uuid.uuid4)
    loan_id = Column(GUID(), ForeignKey("loans.id"), nullable=False)
    benefit_type = Column(String(50), nullable=False)
    applied_at = Column(DateTime, default=datetime.utcnow)
    rp_cost = Column(Integer, nullable=False)
